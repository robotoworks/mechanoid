...
class ContentProviderGenerator {
		def CharSequence generate(Model model, SqliteDatabaseSnapshot snapshot) '''
			...
			import Çmodel.packageNameÈ.AbstractÇmodel.database.name.pascalizeÈOpenHelper.Sources;
			ÇFOR tbl : snapshot.tables.filter([it.hasAndroidPrimaryKey])È
			...
			public abstract class AbstractÇmodel.database.name.pascalizeÈContentProvider extends MechanoidContentProvider {
				...
				static {
					...
					ÇFOR tbl : snapshot.tablesÈ
					sContentTypes[Çtbl.name.underscore.toUpperCaseÈ] = Çmodel.database.name.pascalizeÈContract.Çtbl.name.pascalizeÈ.CONTENT_TYPE;
					ÇIF tbl.hasAndroidPrimaryKeyÈ
					sContentTypes[Çtbl.name.underscore.toUpperCaseÈ_ID] = Çmodel.database.name.pascalizeÈContract.Çtbl.name.pascalizeÈ.ITEM_CONTENT_TYPE;
					ÇENDIFÈ
					ÇENDFORÈ
					ÇFOR vw : snapshot.viewsÈ
					sContentTypes[Çvw.name.underscore.toUpperCaseÈ] = Çmodel.database.name.pascalizeÈContract.Çvw.name.pascalizeÈ.CONTENT_TYPE;
					ÇIF vw.hasAndroidPrimaryKeyÈ
					sContentTypes[Çvw.name.underscore.toUpperCaseÈ_ID] = Çmodel.database.name.pascalizeÈContract.Çvw.name.pascalizeÈ.ITEM_CONTENT_TYPE;
					ÇENDIFÈ
					ÇENDFORÈ					
				}
			    ...
			    protected ContentProviderActions createÇtbl.name.pascalizeÈByIdActions() {
			    	return new DefaultContentProviderActions(Sources.Çtbl.name.underscore.toUpperCaseÈ, true, ÇIF tbl.hasAndroidPrimaryKeyÈÇtbl.name.pascalizeÈRecord.getFactory()ÇELSEÈnullÇENDIFÈ);
			    }
			    
			    ÇENDIFÈ
			    protected ContentProviderActions createÇtbl.name.pascalizeÈActions() {
			    	return new DefaultContentProviderActions(Sources.Çtbl.name.underscore.toUpperCaseÈ, false, ÇIF tbl.hasAndroidPrimaryKeyÈÇtbl.name.pascalizeÈRecord.getFactory()ÇELSEÈnullÇENDIFÈ);
			    }
			    ...
			    protected ContentProviderActions createÇview.name.pascalizeÈByIdActions() {
			    	return new DefaultContentProviderActions(Sources.Çview.name.underscore.toUpperCaseÈ, true, ÇIF view.hasAndroidPrimaryKeyÈÇview.name.pascalizeÈRecord.getFactory()ÇELSEÈnullÇENDIFÈ);
			    }
			    
			    ÇENDIFÈ
			    protected ContentProviderActions createÇview.name.pascalizeÈActions() {
			    	return new DefaultContentProviderActions(Sources.Çview.name.underscore.toUpperCaseÈ, false, ÇIF view.hasAndroidPrimaryKeyÈÇview.name.pascalizeÈRecord.getFactory()ÇELSEÈnullÇENDIFÈ);
			    }
