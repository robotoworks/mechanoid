...
class SqliteMigrationGenerator {
		...
		def CharSequence generate(Model model, MigrationBlock snapshot, int version) '''
			/*
		...
		def CharSequence generateStub(Model model, MigrationBlock snapshot, int version) '''
			...
			import «model.packageName».Abstract«model.database.name.pascalize»MigrationV«version»;
