...
public class OperationServiceBridge {
    ...
    private final Hashtable<String, OperationServiceConfiguration> mConfigurations = new Hashtable<String, OperationServiceConfiguration>();

    private final static SparseArray<Intent> mPendingRequests = new SparseArray<Intent>();
    private final static SparseArray<Intent> mPausedRequests = new SparseArray<Intent>();

    private final Set<OperationServiceListener> mListeners = com.robotoworks.mechanoid.internal.util.Collections
            .newSetFromMap(new WeakHashMap<OperationServiceListener, Boolean>());

    private final OperationLog mLog = new OperationLog(60);
    ...
    /**
     * <p>
     * Helper method to extract an operation id from an operation intent
     * </p>
     * 
        ...
        try {
            ...
            for (ServiceInfo si : services) {
                ...
                if (clz != null && Service.class.isAssignableFrom(clz)) {
                    ...
                    if (field != null) {
                        OperationServiceConfiguration factory = (OperationServiceConfiguration) ReflectUtil
                                .getFieldValueSilently(field);
    ...
    public int execute(Intent intent) {
        ...
        OperationConfiguration opConfig = serviceConfig.getOperationConfigurationRegistry().getOperationConfiguration(
                intent.getAction());
